# Contributing To YeType

Thanks for helping improve YeType. This guide is the contributor entry point for local setup,
validation, and codebase orientation.

YeType is a macOS menu bar app that provides on-device inline autocomplete in other apps. The repo
...
```sh
git clone https://github.com/slyboyshish/YeType.git
cd YeType
open YeType.xcodeproj
```

In Xcode, select the `YeType` scheme. If you run from Xcode, set your signing team under
`Signing & Capabilities`.

## The Xcode Project Is Generated

`YeType.xcodeproj` is generated from `project.yml` by [XcodeGen](https://github.com/yonaskolb/XcodeGen).
It is committed to the repo so you can clone and `open YeType.xcodeproj` without any extra tooling,
but **`project.yml` is the source of truth**.

Source files under `YeType/` and `YeTypeTests/` are auto-discovered by folder, so adding a new
...
CI runs the `XcodeGen` workflow on every PR and fails if the committed `YeType.xcodeproj` differs
...
- `YeType/App/`: app lifecycle, composition root, and top-level coordinators
- `YeType/UI/`: SwiftUI presentation and menu/settings surfaces
- `YeType/Services/`: OS integrations, async work, permissions, and runtime boundaries
- `YeType/Models/`: shared value types, state snapshots, and protocol contracts
- `YeType/Support/`: pure rules, prompt helpers, normalization, and low-level utilities
...
The shared Xcode scheme passes `-yetype-debug` by default in Debug builds. This enables
...
To disable it, uncheck `-yetype-debug` in the scheme's Run → Arguments tab.
