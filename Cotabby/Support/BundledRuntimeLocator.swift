...
/// File overview:
/// Resolves which local model assets YeType should load from user-managed storage.
...
struct BundledRuntimeLocator {
    ...
    static func userRuntimeDirectoryURL(bundle: Bundle = .main) -> URL {
        ...
        let appFolderName =
            (bundle.object(forInfoDictionaryKey: "CFBundleName") as? String) ?? "YeType"
        return
            appSupportRoot
    ...
    /// read-only source, never a replacement: YeType's own writable directory is always scanned and
    ...
    /// YeType's own directory is always first (it is authoritative and the download target); the LM
    ...
    /// source additive: the picker shows YeType's own models plus the LM Studio library together.
    /// Candidate order is preserved (YeType's directory first) and duplicate filenames are deduped,
