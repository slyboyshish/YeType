...
/// File overview:
/// Defines the product-facing engine choices for YeType's autocomplete pipeline.
...
struct SuggestionSettingsSnapshot: Equatable, Sendable {
    ...
    /// gate sees the live value alongside the other "where YeType runs" rules.
    ...
    /// User-authored profile data for YeType's base-model completion prompt.
    ...
    /// When true, YeType checks the current word with `NSSpellChecker` and hides the normal
