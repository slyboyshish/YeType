...
/// File overview:
/// Defines the pure value types that describe YeType's autocomplete domain:
...
struct SuggestionConfiguration: Equatable, Sendable {
    ...
    static let standard = SuggestionConfiguration(
        ...
        // buffer hurts latency with little quality gain because YeType is only completing the
...
enum SuggestionDebugState: Equatable {
    ...
    var detail: String? {
        ...
        case .ready:
            return "Ready means YeType has buffered a non-empty normalized completion for this field and can render it as ghost text."
        }
    }
}
...
///
/// `caretRect` tells YeType where the current insertion point is. `inputFrameRect` gives the
