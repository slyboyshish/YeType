...
End-to-end "land this on main" workflow for YeType. The goal is a **single linear
commit on main** — squash merge, never a merge commit. YeType's `main` ruleset
...
3. **Validate before pushing.** Run the narrowest useful checks, broaden if shared
   ...
   - **XcodeGen:** `project.yml` is the source of truth and `YeType.xcodeproj` is
     generated. New files under `YeType/` and `YeTypeTests/` are auto-discovered —
