# Claude Instructions for YeType

YeType is a community-driven open-source macOS menu bar app that provides
...
- Read the relevant subsystem before editing. YeType is stateful, permission-heavy,
...
- `YeType/App/`: app lifecycle, dependency construction, and coordinators.
- `YeType/UI/`: SwiftUI/AppKit presentation such as settings, onboarding, overlays,
  and menu-facing state.
- `YeType/Services/`: side-effectful boundaries: Accessibility, input monitoring,
  screenshots/OCR, llama runtime, permissions, downloads, updates, and insertion.
- `YeType/Models/`: shared value types, settings snapshots, state machines, and
  protocol contracts.
- `YeType/Support/`: pure rules and low-level helper logic that should be easy to
  test.
- `YeTypeTests/`: focused tests for prompt rendering, request building, availability,
  runtime behavior, and pure state transitions.

## Key Subsystems

- App ownership starts in `YeTypeAppEnvironment` and `AppDelegate`. These construct
...
- `YeType.xcodeproj` is generated from `project.yml` by XcodeGen and committed to the
  repo. `project.yml` is the source of truth. Sources under `YeType/` and `YeTypeTests/`
...
- Wiki lives at https://github.com/slyboyshish/YeType/wiki for contributor onboarding.
...
YeType ships a structured logging system built for AI-assisted debugging. During development
the app is launched with `-yetype-debug`, which enables on-disk JSONL sinks in addition to
the always-on Console.app stream.

**Verbosity floor.** Every handler's level comes from `YeTypeDebugOptions.minimumLogLevel`. The
default is `.info`, which lets swift-log skip per-keystroke `.debug`/`.trace` calls before they
allocate, so they cost nothing in release and do not distort energy measurements. `-yetype-debug`
raises the floor to `.trace`. `YETYPE_LOG_LEVEL=<trace|debug|info|...>` overrides it explicitly,
...
**Log file locations** (only populated when `-yetype-debug` is set):

- `~/Library/Logs/YeType/yetype.jsonl` — main event stream. One JSON object per line. All
...
- `~/Library/Logs/YeType/llm-io.jsonl` — full LLM prompts and completions, one record per
  generation. Shares `request_id` with the main log so a single suggestion can be joined
  across files.
- `~/Desktop/yetype-ax-dump.txt` — most recent Chrome AX tree snapshot. Overwritten on each
...
```bash
jq 'select(.request_id == "req_a3f9k2lq")' ~/Library/Logs/YeType/yetype.jsonl
jq 'select(.request_id == "req_a3f9k2lq")' ~/Library/Logs/YeType/llm-io.jsonl
```
...
jq 'select(.level == "error")' ~/Library/Logs/YeType/yetype.jsonl

# Llama generations slower than 500 ms
jq 'select(.engine == "llama" and .latency_ms > 500)' ~/Library/Logs/YeType/llm-io.jsonl

# Coordinator state transitions
jq 'select(.category == "suggestion" and .stage != null)' ~/Library/Logs/YeType/yetype.jsonl

# Runtime model load/decode events
jq 'select(.category == "runtime")' ~/Library/Logs/YeType/yetype.jsonl
```
...
- Chrome-specific weirdness → start with `~/Desktop/yetype-ax-dump.txt`, then `focus`
- Wrong backend chosen → `suggestion` router selection log (`engine`, `fallback_engine`)

**Console.app fallback** (when `-yetype-debug` wasn't set or the user hasn't relaunched yet):

```bash
log show --predicate 'subsystem == "com.yetype.app"' --last 10m
log stream --predicate 'subsystem == "com.yetype.app"' --level debug
```

Note the default verbosity floor is `.info`, so YeType's `.debug`/`.trace` lines are not emitted
unless the app was launched with `-yetype-debug` or `YETYPE_LOG_LEVEL=debug`. The `--level debug`
flag above controls what `log` *displays*, not what YeType *emits*. The `.info`, warning, and error
...
If `-yetype-debug` clearly isn't on (no JSONL files exist), use the `log show` fallback
...
```bash
xcodebuild -project YeType.xcodeproj -scheme YeType -destination 'platform=macOS' build \
  -derivedDataPath build/DerivedData
xcodebuild -project YeType.xcodeproj -scheme YeType -destination 'platform=macOS' build-for-testing \
  -derivedDataPath build/DerivedData
```
...
`~/Library/Developer/Xcode/DerivedData/YeType-*`, where every build leaves a
