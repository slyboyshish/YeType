...
nonisolated final class LlamaRuntimeCore: @unchecked Sendable {
    private var engine = YeTypeInferenceEngine()
    ...
    func prepare(
        ...
        guard status == .ok else {
            ...
            throw LlamaRuntimeError.unavailable(
                "Unable to load \(resolvedRuntime.modelDisplayName) with YeTypeInferenceEngine."
            )
        }
        ...
        let result = PreparedLlamaRuntime(
            ...
            backendName: "YeTypeInferenceEngine (llama.cpp in-process)"
        )
    ...
    func shutdown(timeoutSeconds: TimeInterval? = nil) {
        ...
        engine.unloadModel()
        preparedRuntime = nil
        YeTypeLogger.runtime.info("Runtime shutdown complete")
