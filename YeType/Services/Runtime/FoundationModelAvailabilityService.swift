...
final class FoundationModelAvailabilityService: ObservableObject {
    ...
    func refresh() {
        ...
        if state != previousState {
            YeTypeLogger.runtime.info("Foundation model availability changed: \(self.state.summary)")
        }
    }
...
protocol FoundationModelAvailabilityProviding {
    ...
    /// Re-reads provider-specific availability and returns it in YeType's app-level vocabulary.
...
/// construct FoundationModels objects and map `SystemLanguageModel.Availability` into YeType's
