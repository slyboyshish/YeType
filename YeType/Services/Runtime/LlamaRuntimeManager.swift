...
/// inside `LlamaRuntimeCore`, a thread-safe class backed by the C++ `YeTypeInferenceEngine`.
@MainActor
final class LlamaRuntimeManager: ObservableObject {
    ...
    func refreshAvailableModels() {
        ...
        YeTypeLogger.runtime.info("Discovered \(self.availableModels.count) model(s)")
    }
    ...
    func configureSelectedModel(filename: String?) {
        ...
        YeTypeLogger.runtime.info("Configured selected model: \(self.selectedModelFilename ?? "none")")
    }
    ...
    func selectModel(filename: String) async throws {
        YeTypeLogger.runtime.info("Selecting model: \(filename)")
    ...
    func generate(
        ...
        do {
            ...
            YeTypeLogger.runtime.debug("Generation cancelled")
            throw LlamaRuntimeError.cancelled
        } catch let error as LlamaRuntimeError {
            YeTypeLogger.runtime.error("Generation runtime error: \(error.localizedDescription)")
            diagnostics.lastError = error.localizedDescription
            throw error
        } catch {
            YeTypeLogger.runtime.error("Generation failed: \(error.localizedDescription)")
    ...
    func stop() {
        YeTypeLogger.runtime.info("Runtime stop requested")
        prepareForStop()
    ...
    private func preparedRuntime() async throws -> PreparedLlamaRuntime {
        ...
        if let cachedRuntime,
            ...
            YeTypeLogger.runtime.trace("Using cached runtime for \(requestedModelFilename)")
        ...
        if let startupTask {
            if startupModelFilename == requestedModelFilename {
                YeTypeLogger.runtime.debug("Reusing in-flight startup for \(requestedModelFilename)")
                return try await awaitPreparedRuntime(startupTask)
            }

            YeTypeLogger.runtime.info("Model changed to \(requestedModelFilename), cancelling previous startup")
            startupTask.cancel()
        ...
        YeTypeLogger.runtime.info("Loading \(resolvedRuntime.modelDisplayName) into memory")
