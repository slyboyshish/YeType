...
/// File overview:
/// Adapts Apple's on-device Foundation Models framework to YeType's existing
...
final class FoundationModelSuggestionEngine {
        ...
        do {
            ...
            guard let model = availabilityService.systemLanguageModel else {
                throw SuggestionClientError.unavailable(
                    "Apple Intelligence reported available, but YeType could not access the system language model."
                )
            }
            ...
            YeTypeLogger.suggestion.debug("Foundation model generation cancelled", metadata: baseMetadata)
            throw SuggestionClientError.cancelled
        } catch let error as LanguageModelSession.GenerationError {
            YeTypeLogger.suggestion.error(
    ...
    func prewarm(for request: SuggestionRequest) async {
        ...
        let session = ensureSession(for: request, model: model)
        session.prewarm()
        YeTypeLogger.suggestion.debug("Foundation model session prewarmed")
    }
    ...
    /// Maps YeType's existing generation knobs onto the subset of Foundation Models options the
    ...
    /// Converts framework-specific failures into YeType's existing error vocabulary so the rest of
    ...
    private func mapGenerationError(
        ...
        case .unsupportedGuide:
            return .generationFailed("Apple Intelligence rejected a guided-generation request YeType sent.")
        case .decodingFailure:
            return .generationFailed("Apple Intelligence returned a response YeType could not decode.")
        case .rateLimited:
