...
/// File overview:
/// Owns YeType's Sparkle integration and keeps updater lifecycle out of SwiftUI views.
...
final class AppUpdateManager {
    ...
    private static let debugCheckForUpdatesOnLaunchArgument = "-YeType-check-for-updates-on-launch"
    ...
    func start() {
        ...
        guard Self.isUpdaterEnabledForThisBuild else {
            // Dev builds carry a distinct bundle identifier (`com.kludne.yetype.dev`) so they hold
    ...
    /// (the `YETYPE_DEV` flag), which ships under a distinct bundle identifier that the prod appcast
    ...
    private func log(_ message: String) {
        YeTypeLogger.updates.info("\(message)")
    }
}
