...
final class ModelDownloadManager: ObservableObject {
    ...
    /// YeType's own directory first (authoritative + download target), then any additive sources
    ...
    /// The path shown in Settings and opened by "Open Folder". This is always YeType's own writable
    ...
    func download(_ model: DownloadableRuntimeModel) {
        guard downloadTasks[model.filename] == nil else {
            YeTypeLogger.models.debug("Download already in progress for \(model.filename)")
            return
        }

        if isInstalled(model: model) {
            YeTypeLogger.models.debug("Model \(model.filename) already installed, skipping download")
            modelStates[model.filename] = .downloaded
            return
        }

        YeTypeLogger.models.info("Starting download for \(model.filename)")
    ...
    /// Returns `true` only when the model lives in YeType's user-writable model directory.
    ...
    func deleteModel(filename: String) {
        ...
        do {
            ...
            YeTypeLogger.models.error("Failed to delete model \(filename): \(error.localizedDescription)")
        }
    }
    ...
    private func performDownload(_ model: DownloadableRuntimeModel) async {
        ...
        do {
            ...
            YeTypeLogger.models.info("Download complete for \(model.filename)")
            ...
            if DownloadOutcomeClassifier.isUserCancellation(error) {
                YeTypeLogger.models.info("Download cancelled by user for \(model.filename)")
                modelStates[model.filename] = isInstalled(model: model) ? .downloaded : .idle
            } else {
                YeTypeLogger.models.error("Download failed for \(model.filename): \(error.localizedDescription)")
