...
/// File overview:
/// Tracks YeType's own synthetic key events so inserted suggestions do not recursively trigger
/// the input-monitoring pipeline and cause bogus follow-up completions.
///
/// Think of this as a tiny "ignore my own write" guard. When YeType injects accepted text back into
...
final class InputSuppressionController {
    ...
    /// Stamped onto YeType's synthetic insertion events so any tap can recognize them by identity,
    ...
    /// Arms a short-lived suppression window for the synthetic keydown events YeType is about to post.
    func registerSyntheticInsertion(expectedKeyDownCount: Int) {
        ...
        YeTypeLogger.app.trace("Suppression armed for \(expectedKeyDownCount) synthetic key event(s)")
    }
    ...
    /// Tags an event YeType is about to post so taps can ignore it by identity.
    func markSynthetic(_ event: CGEvent) {
        event.setIntegerValueField(.eventSourceUserData, value: Self.syntheticEventUserData)
    }

    /// True when the event carries YeType's synthetic-insertion marker.
