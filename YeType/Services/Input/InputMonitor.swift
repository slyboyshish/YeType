...
/// and `Tab` acceptance. This is the boundary between raw CGEvents and YeType's smaller
...
///
/// `notHandled` means the key was not one of YeType's configured accept keys. `passThrough`
/// means it was an accept key but YeType declined to consume it. `consume` means the coordinator
...
final class InputMonitor {
    ...
    /// coordinator. This eliminates per-keystroke overhead in apps where YeType will never act
    ...
    /// becomes visible or hidden, so YeType only enters the synchronous event path while there is
    /// something to accept.
    ...
    private func installObserverTapIfNeeded() {
        ...
        guard let tap = CGEvent.tapCreate(
            ...
            YeTypeLogger.app.warning("Failed to create CGEvent observer tap — Input Monitoring permission may be missing")
            return
        }
        YeTypeLogger.app.info("CGEvent observer tap installed (listen-only)")
    ...
    private func installAcceptTapIfNeeded() {
        ...
        guard let tap = CGEvent.tapCreate(
            ...
            YeTypeLogger.app.warning("Failed to create CGEvent accept tap")
            return
        }
        YeTypeLogger.app.info("CGEvent accept tap installed (active, accept-key only)")
    ...
    private func installToggleTapIfNeeded() {
        ...
        guard let tap = CGEvent.tapCreate(
            ...
            YeTypeLogger.app.warning("Failed to create CGEvent toggle tap")
            return
        }
        YeTypeLogger.app.info("CGEvent toggle tap installed (active, toggle-hotkey only)")
    ...
    private func destroyAcceptTap() {
        ...
        acceptTap = nil
        YeTypeLogger.app.info("CGEvent accept tap removed")
    }
    ...
    private func destroyToggleTap() {
        ...
        toggleTap = nil
        YeTypeLogger.app.info("CGEvent toggle tap removed")
    }
    ...
    /// `shouldProcessEventsProvider` because the hotkey must work even when YeType is globally
    /// disabled — that is its only job.
    func handleToggleTap(type: CGEventType, event: CGEvent) -> Unmanaged<CGEvent>? {
        switch type {
        case .tapDisabledByTimeout, .tapDisabledByUserInput:
            YeTypeLogger.app.warning("Toggle tap was disabled by system, re-enabling")
            if let toggleTap {
        ...
        case .keyDown:
            ...
            YeTypeLogger.app.debug("Toggle tap consumed keyCode=\(keyCode) modifiers=\(modifiers.rawValue)")
            return nil
    ...
    func handleObserverTap(type: CGEventType, event: CGEvent) -> Unmanaged<CGEvent>? {
        switch type {
        case .tapDisabledByTimeout, .tapDisabledByUserInput:
            YeTypeLogger.app.warning("Observer tap was disabled by system, re-enabling")
            if let observerTap {
        ...
        case .keyDown:
            ...
            // Short-circuit before classification when YeType won't act on events for the
    ...
    func handleAcceptTap(type: CGEventType, event: CGEvent) -> Unmanaged<CGEvent>? {
        switch type {
        case .tapDisabledByTimeout, .tapDisabledByUserInput:
            YeTypeLogger.app.warning("Accept tap was disabled by system, re-enabling")
            if let acceptTap {
        ...
        case .keyDown:
            ...
            // it recognizes YeType's synthetic insertion events by identity instead. Without this an
    ...
    func handleAcceptKeyDown(_ keyEvent: InputMonitorKeyEvent) -> InputMonitorAcceptTapDecision {
        ...
        guard shouldConsumeAcceptKeyProvider() else {
            ...
            YeTypeLogger.app.debug("\(message)")
            return .passThrough
        }

        guard let onEvent else {
            YeTypeLogger.app.debug("Accept tap declining to consume keyCode=\(keyEvent.keyCode): no event handler")
            return .passThrough
        }
