...
/// File overview:
/// Coordinates YeType's guided permission flow.
///
...
final class PermissionGuidanceController {
    ...
    private var activePermission: YeTypePermissionKind?
    ...
    func requestAccess(for permission: YeTypePermissionKind, sourceFrameInScreen: CGRect? = nil) {
    ...
    private func presentGuidance(for permission: YeTypePermissionKind, sourceFrameInScreen: CGRect?) {
        dismiss()
        permissionManager.refresh()
        guard !permissionManager.isGranted(permission) else {
            return
    ...
    private func refreshPosition() {
        ...
        // Settings, the macOS permission dialog, and YeType's own windows.
