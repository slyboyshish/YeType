...
/// File overview:
/// Emits high-signal model-boundary logs for the suggestion pipeline when `-yetype-debug` is
/// enabled. The logger intentionally focuses on the payloads that explain model behavior:
/// the prompt YeType sent, the raw model response, and the normalized response YeType may display.
///
...
final class SuggestionDebugLogger {
    ...
    private func logLine(_ line: String, color: String? = nil) {
        ...
        YeTypeDebugOptions.log(styled(line, color: color))
    }
