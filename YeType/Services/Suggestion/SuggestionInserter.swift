...
final class SuggestionInserter {
    ...
    private static let pasteInsertionDefaultsKey = "yetypePasteInsertionEnabled"
    ...
    func insert(_ suggestion: String) -> Bool {
        ...
        guard !normalized.isEmpty else {
            ...
            YeTypeLogger.suggestion.warning("Insertion skipped: suggestion was empty after normalization")
            return false
        }
        ...
        if InsertionStrategySelector.strategy(
            ...
            YeTypeLogger.suggestion.debug("Inserted \(normalized.count) characters via clipboard paste")
            return true
        }
        ...
        guard let keyDownEvent = CGEvent(keyboardEventSource: nil, virtualKey: 0, keyDown: true),
            ...
            YeTypeLogger.suggestion.error("Failed to create synthetic keyboard events for insertion")
            return false
        }
        ...
        lastErrorMessage = nil
        YeTypeLogger.suggestion.debug("Inserted \(normalized.count) characters via synthetic keystroke")
        return true
    }
    ...
    func replace(deletingUTF16Count: Int, with text: String) -> Bool {
        ...
        for _ in 0..<plan.backspaceCount {
            guard let down = CGEvent(keyboardEventSource: nil, virtualKey: Self.backspaceKeyCode, keyDown: true),
                ...
                YeTypeLogger.suggestion.error("Replace failed: could not create delete events")
                return false
            }
            events.append(down)
            events.append(up)
        ...
        if !plan.insertUTF16.isEmpty {
            guard let down = CGEvent(keyboardEventSource: nil, virtualKey: 0, keyDown: true),
                ...
                lastErrorMessage = "Unable to create a synthetic keyboard event."
                YeTypeLogger.suggestion.error("Replace failed: could not create insertion events")
                return false
            }
