...
/// `~/Desktop/yetype-ax-dump.txt`. This is a developer diagnostic for triaging caret-placement and
...
///
/// The dump only runs on debug builds (`-yetype-debug`), only for the configured bundle, and is
...
enum AXTreeDumpWriter {
    /// Bundle identifier we automatically dump the AX tree for when `-yetype-debug` is on.
    ...
    static func dumpIfEnabled(
        ...
        guard YeTypeDebugOptions.isEnabled,
    ...
    /// `~/Desktop/yetype-ax-dump.txt`. The file is overwritten so the user (or an AI debugger)
    /// always inspects the latest snapshot at a stable path.
    ///
    /// Writes are best-effort: a failed disk write is logged through `YeTypeLogger.focus` and
    ...
    private static func writeAXTreeDumpToDesktop(focusedElement: AXUIElement, app: String, bundle: String) {
        ...
        guard let desktopURL = FileManager.default
            ...
            YeTypeLogger.focus.error("AX dump skipped: no Desktop directory available")
            return
        }
        let targetURL = desktopURL.appendingPathComponent("yetype-ax-dump.txt", isDirectory: false)
        do {
