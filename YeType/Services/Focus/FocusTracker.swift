...
/// loop gives YeType predictable eventual consistency: every tick re-reads the current frontmost
...
final class FocusTracker {
    ...
    /// AX identifier of the one element inside YeType's own UI that is allowed to be captured: the
    ...
    /// element in YeType's windows stays blocked.
    ...
    /// disabling YeType globally or for a specific app must actually stop the walk, not just
    ...
    func start() {
        ...
        YeTypeLogger.focus.info("Focus polling started at \(Int(self.pollInterval * 1000))ms interval")
    ...
    func stop() {
        YeTypeLogger.focus.info("Focus polling stopped")
        timer?.invalidate()
        timer = nil
    ...
    func updatePollInterval(_ interval: TimeInterval) {
        ...
        YeTypeLogger.focus.info("Focus poll interval changed to \(Int(interval * 1000))ms")
    ...
    private func captureSnapshotRaw() -> FocusCaptureResult {
        ...
        // YeType never completes inside its own UI, with one sanctioned exception: the Context pane's
        // live-preview field, tagged with a known AX identifier so the user can exercise the real
        // pipeline against their settings. Every other element in YeType's own windows (search field,
        // Extended Context editor, menus) stays blocked, so this cannot leak completions into Settings.
        // The identifier AX read is an autoclosure, so it runs only when YeType itself is focused.
        if !SelfCaptureGate.allowsCapture(
            ...
            return inactiveCapture(
                applicationName: application.localizedName ?? "YeType",
                bundleIdentifier: application.bundleIdentifier,
                capability: .blocked("YeType is focused.")
            )
        }

        // Bail before any AX deep-walk when YeType is disabled for the focused app. Stops
        ...
        if isCaptureSuppressedForBundle(application.bundleIdentifier) {
            ...
            return inactiveCapture(
                ...
                bundleIdentifier: application.bundleIdentifier,
                capability: .blocked("YeType is disabled for this app.")
            )
        }
    ...
    private func logChromeFocusProbe(source: String, application: NSRunningApplication) {
        guard YeTypeDebugOptions.isEnabled else { return }
    ...
    /// cache hit/miss tally. Gated behind `-yetype-debug`. This is the signal that distinguishes
    ...
    private func logResolveTiming(
        ...
        guard YeTypeDebugOptions.isEnabled else {
            return
        }
    ...
    private func noteCaptureSuppressed(for application: NSRunningApplication) {
        ...
        YeTypeLogger.focus.info("Focus capture suppressed for \(name) (\(id))")
    }
    ...
    private func noteCaptureResumedIfNeeded() {
        ...
        YeTypeLogger.focus.info("Focus capture resumed")
    }
