...
struct FocusSnapshotResolver {
    ...
    func resolveSnapshot(
        ...
        guard let resolvedCandidate = candidateResolution.resolvedCandidate else {
            YeTypeLogger.focus.trace("Focus unsupported in \(applicationName): \(resolution.unsupportedReason)")
    ...
    /// Extracts the AX properties YeType needs from one candidate element near the current focus.
    ...
    /// Detects secure inputs so YeType can intentionally refuse to operate in sensitive fields.
