...
final class OverlayController: SuggestionOverlayControlling {
    ...
    private lazy var panel: OverlayPanel = {
        ...
        // A non-activating panel lets YeType draw UI near the caret without stealing focus
    ...
    private func showInline(
        ...
        guard AXHelper.rectHasFiniteComponents(frame) else {
            YeTypeLogger.suggestion.warning("Skipped inline overlay: computed a non-finite frame")
            return
        }
    ...
    /// Mirror-mode rendering. Draws the suggestion inside a YeType-owned card anchored to the
    ...
    private func showMirror(
        ...
        guard AXHelper.rectHasFiniteComponents(panelFrame) else {
            YeTypeLogger.suggestion.warning("Skipped mirror overlay: computed a non-finite frame")
            return
        }
...
/// Mirror-mode card. Renders the suggestion inside a YeType-owned backdrop anchored below the
