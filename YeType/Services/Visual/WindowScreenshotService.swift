...
struct WindowScreenshotService: WindowScreenshotCapturing {
    ...
    func captureSnapshot(
        ...
        guard CGPreflightScreenCaptureAccess() else {
            YeTypeLogger.app.warning("Screenshot blocked: Screen Recording permission missing")
        ...
        guard let matchingWindow else {
            YeTypeLogger.app.debug("No visible window for pid \(processIdentifier)")
        ...
        YeTypeLogger.app.trace("Capturing window: \(windowTitle) (\(windowWidth)x\(windowHeight))")

        // Capture the ENTIRE display the focused window is on, so OCR context includes the whole
        // working screen — the full app in fullscreen mode, or every visible window in windowed
        // mode — instead of only a narrow band around the caret. The captured image is downsampled
        // by `ScreenTextExtractor` (maxImageDimension) before OCR, so full-display capture stays
        // light on a fanless Mac.
        let display = shareableContent.displays.first(where: {
            $0.frame.intersects(matchingWindow.frame)
        }) ?? shareableContent.displays.first

        guard let display else {
            YeTypeLogger.app.debug("No shareable display found for window capture")
            throw WindowScreenshotError.captureFailed("No shareable display available.")
        }

        // Exclude YeType's own windows (ghost-text overlay, settings panels) from the capture so
        // the model never reads its own suggestions back in as context.
        let ownApplication = shareableContent.applications.first(where: {
        ...
        YeTypeLogger.app.trace("Capturing full display \(display.width)x\(display.height) for window: \(windowTitle)")
