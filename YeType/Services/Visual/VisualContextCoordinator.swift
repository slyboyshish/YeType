...
final class VisualContextCoordinator {
    ...
    private func launchSession(for snapshotContext: FocusedInputSnapshot) {
        YeTypeLogger.app.debug("Starting visual context session for element \(snapshotContext.elementIdentifier)")
        ...
        visualContextTask = Task { [weak self] in
            ...
            do {
                ...
                YeTypeLogger.app.debug("Visual context generation cancelled")
                return
            } catch let error as ScreenshotContextGenerationError {
                YeTypeLogger.app.warning("Visual context generation error: \(error.localizedDescription)")
                setStatus(errorStatus(for: error), for: session.sessionID)
            } catch {
                YeTypeLogger.app.error("Visual context generation failed: \(error.localizedDescription)")
    ...
    private func applyExcerpt(
        ...
        YeTypeLogger.app.debug("Visual context ready: \(excerpt.text.count) chars")
        publishState()
