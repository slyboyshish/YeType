...
struct ContextPaneView: View {
    ...
    private var livePreviewSection: some View {
        Section("Live preview") {
            VStack(alignment: .leading, spacing: 12) {
                Text("Type below and YeType completes as you go, using the same engine and settings " +
    ...
    private var howThisIsUsedSection: some View {
        Section("How this is used") {
            VStack(alignment: .leading, spacing: 8) {
                ...
                bulletLine(
                    "Subordinate to YeType's base autocomplete rules, so it cannot override " +
                        "core behavior."
                )
                bulletLine(
