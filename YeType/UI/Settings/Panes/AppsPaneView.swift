...
/// File overview:
/// "Apps" detail pane of the redesigned Settings window. Lists every app where YeType is
...
struct AppsPaneView: View {
    ...
    var body: some View {
        SettingsPaneScaffold {
            Section("Disabled Apps") {
                Text("YeType won't autocomplete in these apps. Add an app you can't disable from the "
                    + "menu bar, like a launcher that closes the moment it loses focus.")
                    .font(.caption)
                    .foregroundStyle(.secondary)

                if suggestionSettings.disabledAppRules.isEmpty {
                    Text("No apps are disabled. YeType is active in every supported field.")
                        .font(.callout)
                        .foregroundStyle(.secondary)
                } else {
    ...
    /// Lets the user disable YeType in an app they can't reach from the menu bar. The menu-bar
    ...
    private func presentDisabledAppPicker() {
        ...
        panel.message = "Choose apps where YeType should not autocomplete."
...
private struct RunningAppSuggestion: Identifiable {
    ...
    /// Snapshot the user-launched apps (`activationPolicy == .regular`) excluding YeType itself,
