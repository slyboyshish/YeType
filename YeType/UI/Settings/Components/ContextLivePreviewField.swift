...
///
/// YeType never completes inside its own UI (`FocusTracker` blocks capture whenever YeType is the
...
enum ContextLivePreview {
    ...
    /// self-capture for this element and nothing else in YeType's own windows. Wired into the focus
    /// pipeline in `YeTypeAppEnvironment`.
    static let accessibilityIdentifier = "com.yetype.settings.context.live-preview"
}
...
struct ContextLivePreviewField: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> NSScrollView {
        ...
        // this sanctioned field apart from every other element in YeType's own windows.
