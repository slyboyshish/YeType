...
struct PermissionReminderView: View {
    ...
    var body: some View {
        VStack(spacing: 28) {
            ...
            VStack(spacing: 10) {
                ForEach(YeTypePermissionKind.allCases.filter(\.isRequiredForAutocomplete)) { permission in
                ...
                ForEach(YeTypePermissionKind.allCases.filter(\.isOptionalEnhancement)) { permission in
                    ReminderPermissionCard(
                        permission: permission,
                        granted: permissionManager.isGranted(permission),
                        isOptional: true,
