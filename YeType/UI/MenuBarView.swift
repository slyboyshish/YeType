...
/// File overview:
/// Composes YeType's primary menu-bar control panel as a status-first surface
...
struct MenuBarView: View {
    ...
    /// Lists every permission YeType can use and appears whenever at least one is missing, including
    ...
    /// `YeTypePermissionKind.isRequiredForAutocomplete`).
    @ViewBuilder
    private var permissionsCard: some View {
        if !allPermissionsGranted {
            VStack(alignment: .leading, spacing: 6) {
                ...
                ForEach(YeTypePermissionKind.allCases) { permission in
                    PermissionRow(
