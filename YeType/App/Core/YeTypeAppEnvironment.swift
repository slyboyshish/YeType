...
/// File overview:
/// Builds YeType's long-lived dependency graph in one place. This is the app's composition model:
...
@MainActor
final class YeTypeAppEnvironment {
    ...
    init() {
        YeTypeLogger.app.info("Building dependency graph")
        ...
        // Stop the deep AX walk when YeType is disabled for the focused app. Without this the
        // focus poll keeps enumerating the frontmost app's AX attributes every 50-80ms even after
        // the user toggles YeType off, which can dismiss transient popovers in apps like Calendar
        ...
        let focusModel = FocusTrackingModel(
            ...
            // The Context pane's live-preview field is the single sanctioned spot where YeType may
        ...
        if #available(macOS 26.0, *) {
            ...
            YeTypeLogger.app.info("Foundation model engine available")
        } else {
            foundationModelEngine = UnavailableSuggestionEngine(
                message: foundationModelAvailabilityService.userVisibleMessage
            )
            YeTypeLogger.app.info("Foundation model engine unavailable (macOS version)")
        }
        #else
        foundationModelEngine = UnavailableSuggestionEngine(
            message: foundationModelAvailabilityService.userVisibleMessage
        )
        YeTypeLogger.app.info("Foundation model engine unavailable (SDK)")
        #endif
