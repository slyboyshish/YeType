...
/// YeType's compact status UI. Shared services are injected through `AppDelegate`.
///
/// `@main` marks the single process entry point for a Swift app.
@main
struct YeTypeApp: App {
    /// Bridges old-style AppKit lifecycle callbacks into a SwiftUI app.
    @NSApplicationDelegateAdaptor(AppDelegate.self) private var appDelegate

    /// Defines the menu bar extra that surfaces YeType's runtime, focus, and suggestion state.
    var body: some Scene {
        MenuBarExtra {
            MenuBarView(
                ...
                onReportFeedback: {
                    guard let baseURL = URL(string: "https://github.com/slyboyshish/YeType/issues") else {
                        return
                    }
                    // Attach host details so the landing form can pre-fill the Environment block
                    // (YeType + macOS + hardware) and the user only has to write the actual report.
