...
/// `YeTypeAppEnvironment`, while `AppDelegate` focuses on lifecycle wiring and cross-subsystem
/// subscriptions.
///
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private let environment: YeTypeAppEnvironment
    private var cancellables = Set<AnyCancellable>()
    private var didStartServices = false

    override init() {
        YeTypeLogger.bootstrap()
        ...
        let environment = YeTypeAppEnvironment()
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        guard !Self.isRunningUnderXCTest else {
            YeTypeLogger.app.info("Unit test host detected; skipping production service startup")
            return
        }

        let version = Bundle.main.object(forInfoDictionaryKey: "CFBundleShortVersionString") as? String ?? "?"
        let build = Bundle.main.object(forInfoDictionaryKey: "CFBundleVersion") as? String ?? "?"
        YeTypeLogger.app.info("YeType \(version) (build \(build)) launching on macOS \(ProcessInfo.processInfo.operatingSystemVersionString)")
        ...
        YeTypeLogger.app.info("All services started")
    }
    ...
    private func applyLaunchAtLoginDefaultIfNeeded() {
        let appliedKey = "yetypeLaunchAtLoginDefaultApplied"
        let defaults = UserDefaults.standard
        guard !defaults.bool(forKey: appliedKey) else { return }
        LaunchAtLogin.isEnabled = true
        defaults.set(true, forKey: appliedKey)
        YeTypeLogger.app.info("Applied default Open at Login = true (one-time)")
    }
    ...
    func applicationWillTerminate(_ notification: Notification) {
        ...
        YeTypeLogger.app.info("YeType terminating, releasing services")
        activationIndicatorController.hide(reason: "Activation indicator hidden because YeType is terminating.")
    ...
    /// Shows or hides the field-edge YeType icon based on focus state, global enable, per-app
