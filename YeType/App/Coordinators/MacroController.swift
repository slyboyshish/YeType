...
final class MacroController {
    ...
    private func beginCapture() {
        guard canTrigger(), let context = focusModel.snapshot.context else {
            ...
            YeTypeLogger.suggestion.debug("macro capture aborted at open: no triggerable focus context")
            machine.reset()
            return
        }
    ...
    private func commitIfPossible() {
        ...
        YeTypeLogger.suggestion.debug("macro commit deleteUTF16=\(deleteCount) query=\"\(currentQuery)\"")
        teardownCapture()
