...
extension SuggestionCoordinator {
    ...
    private func acceptSuggestion(
        ...
        guard permissionManager.inputMonitoringGranted else {
            return passTabThrough(
                reason: "Input Monitoring permission is required before YeType can accept suggestions."
            )
        }
    ...
    /// How long YeType keeps owning Tab after a final-chunk accept while it waits for the
    ...
    func logStage(
        ...
        YeTypeLogger.suggestion.debug(.init(stringLiteral: message), metadata: metadata)
    }
}
