...
final class EmojiPickerController {
    ...
    private func beginCapture(query: String) {
        guard canTrigger(), let context = focusModel.snapshot.context else {
            ...
            YeTypeLogger.suggestion.debug("emoji capture aborted at open: no triggerable focus context")
            machine.reset()
            return
        }
        ...
        YeTypeLogger.suggestion.debug("emoji capture opened query=\"\(query)\" matches=\(matches.count)")
    }
    ...
    private func commitSelectedMatch() {
        guard selectedIndex >= 0, selectedIndex < matches.count else {
            YeTypeLogger.suggestion.debug("emoji commit (key) skipped: no selectable match query=\"\(currentQuery)\"")
            cancelCapture()
            return
        }
        ...
        YeTypeLogger.suggestion.debug("emoji commit (key) glyph=\(glyph) query=\"\(currentQuery)\"")
        teardownCapture()
    ...
    private func handleFocusSnapshot(_ snapshot: FocusSnapshot) {
        ...
        guard let context = snapshot.context,
            ...
            YeTypeLogger.suggestion.debug("emoji capture cancelled: focus changed during capture")
            cancelCapture()
            return
        }
