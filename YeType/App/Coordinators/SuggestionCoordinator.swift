...
/// File overview:
/// Declares the shared state and dependency graph for YeType's inline-completion orchestrator.
...
final class SuggestionCoordinator: ObservableObject {
    ...
    /// `YeTypeAppEnvironment`) so the underlying `NSSpellChecker` document tag persists across the
    ...
    static let totalTabAcceptedWordCountDefaultsKey = "yetypeTotalAcceptedWordCount"
    ...
    /// True while YeType keeps the accept tap owning Tab in the gap between a final-chunk accept and
    ...
    init(
        ...
        overlayController.onStateChange = { [weak self] state in
            ...
            // visible. With the overlay hidden, YeType observes via a listen-only tap that does
