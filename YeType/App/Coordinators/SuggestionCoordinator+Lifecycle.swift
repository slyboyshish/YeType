...
extension SuggestionCoordinator {
    ...
    func start() {
        YeTypeLogger.suggestion.info("Suggestion coordinator starting")
        reconcileWithCurrentEnvironment()
    }

    /// Cancels any pending work and detaches long-lived callbacks during shutdown.
    func stop() {
        YeTypeLogger.suggestion.info("Suggestion coordinator stopping")
        cancelPredictionWork()
        resetCachedGenerationContext()
        visualContextCoordinator.cancel(resetState: true)
        hideOverlay(reason: "Overlay hidden because YeType stopped observing suggestions.")
    ...
    func prepareForRuntimeModelSwitch() {
        YeTypeLogger.suggestion.info("Preparing for runtime model switch, clearing active state")
        cancelPredictionWork()
        resetCachedGenerationContext()
    ...
    func handleSuggestionSettingsChange(_ snapshot: SuggestionSettingsSnapshot) {
        ...
        YeTypeLogger.suggestion.info("Settings changed, resetting suggestion state")
