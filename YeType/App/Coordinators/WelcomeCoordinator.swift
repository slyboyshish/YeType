...
final class WelcomeCoordinator: NSObject, NSWindowDelegate {
    ...
    ///
    /// Replaces the legacy boolean `yetypeOnboardingCompleted` key. That key is intentionally not
    ...
    private static let onboardingCompletedVersionKey = "yetypeOnboardingCompletedVersion"
    ...
    /// relaunches YeType, which lands before the final "Start Using YeType" tap that stamps
    ...
    private static let onboardingProgressStepKey = "yetypeOnboardingProgressStep"
    ...
    /// (taps "Start Using YeType"), not when the window first appears. If the user closes the
    ...
    func showWelcome() {
        ...
        window.title = "Welcome to YeType"
    ...
    /// Called when the user completes the full onboarding wizard ("Start Using YeType"). Stamps the
    ...
    private func showPermissionReminder() {
        ...
        window.title = "YeType — Permissions"
        window.titleVisibility = .hidden
        window.titlebarAppearsTransparent = true
        window.isMovableByWindowBackground = true
        window.center()
