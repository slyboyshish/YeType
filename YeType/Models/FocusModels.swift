...
/// These types let the rest of YeType reason about focus without depending on raw Accessibility values.
...
///
/// These are the concrete Accessibility capabilities YeType needs before it can safely assist a field.
...
/// Blocked means YeType intentionally refuses to operate, for example in secure fields.
...
struct FocusSnapshot: Equatable {
    ...
    ///
    /// Opening YeType's menu bar window can briefly make YeType the focused app. Treating YeType's own
...
/// Minimal identity for the last non-YeType application the user was working in.
///
/// The menu bar panel can steal focus when opened, so UI controls that target "the current app"
/// need a stable application identity that does not immediately collapse to YeType's own process.
