...
/// `~/Library/Logs/YeType/`. This is the on-disk sink an AI debugging agent can read
/// directly without copy-pasting from Console.app.
///
/// The handler is only installed when `-yetype-debug` is passed at launch (see
/// `YeTypeLogger.bootstrap`). When the file grows past `sizeCapBytes` it is rotated: the
/// current file becomes `yetype.jsonl.1` (overwriting any prior rotation) and a fresh empty
/// `yetype.jsonl` is opened. The previous truncate-to-zero behavior threw away the *most
...
final class FileLogWriter: @unchecked Sendable {
    ...
    private static func makeLogFileURL() -> URL? {
        ...
        try? fileManager.createDirectory(at: directory, withIntermediateDirectories: true)
        return directory.appendingPathComponent("yetype.jsonl")
    }
}
...
struct FileLogHandler: LogHandler {
    ...
    /// `logLevel` defaults to `YeTypeDebugOptions.minimumLogLevel`. This sink is only installed
    /// under `-yetype-debug`, where the floor is `.trace`, so it captures everything by default —
    /// but sourcing the level from the same place keeps it honest if the floor is overridden via
    /// `YETYPE_LOG_LEVEL`.
    init(
        label: String,
        writer: FileLogWriter = .shared,
        logLevel: Logging.Logger.Level = YeTypeDebugOptions.minimumLogLevel
    ) {
        self.label = label
        self.writer = writer
    ...
    /// `com.yetype.runtime` → `runtime`. Matches `OSLogHandler`'s category convention so the
