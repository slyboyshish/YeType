...
///
/// Kept separate from `yetype.jsonl` so the main event stream stays readable: a single generation
...
///
/// One record per generation, written to `~/Library/Logs/YeType/llm-io.jsonl`. Records share a
/// `request_id` with the main log so a debugger can join across files.
///
/// Like `FileLogHandler`, this handler is only installed when `-yetype-debug` is set, so a release
...
/// handler does not inspect log levels because every event routed to `YeTypeLogger.llmIO` is
/// intentional.
