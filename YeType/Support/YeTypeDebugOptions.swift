...
/// File overview:
/// Centralizes YeType's developer-only runtime switches.
///
/// A single launch argument is easier to reason about than separate feature flags because every
/// privacy-sensitive diagnostic path has one obvious gate. Passing `-yetype-debug` means the
...
enum YeTypeDebugOptions {
    static let launchArgument = "-yetype-debug"
    ...
    /// 1. `YETYPE_LOG_LEVEL=<trace|debug|info|notice|warning|error|critical>` — explicit override,
    ///    e.g. to get Console `.debug` output without the heavier `-yetype-debug` file/screenshot
    ///    artifacts. An unrecognized value is ignored.
    /// 2. `-yetype-debug` — full `.trace` capture to Console and the JSONL sinks.
    /// 3. Default — `.info`.
    static var minimumLogLevel: Logging.Logger.Level {
        if let raw = ProcessInfo.processInfo.environment["YETYPE_LOG_LEVEL"]?.lowercased(),
    ...
    static func log(_ message: @autoclosure () -> String) {
        ...
        YeTypeLogger.debug.debug("\(message())")
    }
}
...
/// subsystem as a filterable column. When the `-yetype-debug` launch argument is set we
/// additionally fan out to `FileLogHandler`, which writes JSONL to
/// `~/Library/Logs/YeType/yetype.jsonl` for AI-assisted debugging without copy-paste.
enum YeTypeLogger {
    ...
    /// and would dominate Console.app; kept out of `yetype.jsonl` because it would drown the
    /// orchestration signal an AI debugger wants to skim.
    static let llmIOLabel = "com.yetype.llm-io"
    ...
    private static let bootstrapOnce: Void = {
        ...
        let installFileHandler = YeTypeDebugOptions.isEnabled
    ...
    private static func announceConfiguration(fileSinksInstalled: Bool) {
        var metadata: Logging.Logger.Metadata = [
            "debug_mode": .stringConvertible(YeTypeDebugOptions.isEnabled),
            "min_log_level": .string(YeTypeDebugOptions.minimumLogLevel.rawValue),
    ...
    static let app = Logger(label: "com.yetype.app")
    static let debug = Logger(label: "com.yetype.debug")
    static let runtime = Logger(label: "com.yetype.runtime")
    static let focus = Logger(label: "com.yetype.focus")
    static let updates = Logger(label: "com.yetype.updates")
    static let models = Logger(label: "com.yetype.models")
    static let suggestion = Logger(label: "com.yetype.suggestion")
    /// Full prompts and completions, one structured JSON record per generation. Writes to
    /// `~/Library/Logs/YeType/llm-io.jsonl` only when `-yetype-debug` is set.
...
struct OSLogHandler: LogHandler {
    ...
    private static let subsystem = "com.yetype.app"

    /// `logLevel` defaults to `YeTypeDebugOptions.minimumLogLevel` so the always-on Console sink is
    /// quiet (`.info`) in normal runs and fully verbose (`.trace`) under `-yetype-debug`. The floor
    /// is what lets swift-log skip per-keystroke `.debug`/`.trace` calls before they allocate.
    init(label: String, logLevel: Logging.Logger.Level = YeTypeDebugOptions.minimumLogLevel) {
