...
```bash
xcodegen generate
xcodebuild -project Cotabby.xcodeproj -scheme "Cotabby" -configuration Debug build
```

## Credits & License
YeType is a fork of [Cotabby](https://github.com/FuJacob/cotabby) by FuJacob, built on the
[CotabbyInference](https://github.com/FuJacob/cotabbyinference) runtime (llama.cpp). Thanks to the original authors.

Licensed under **AGPL-3.0** (see [LICENSE](LICENSE)) — same license as upstream.
