...
/// Tests for `SelfCaptureGate`, the rule that keeps YeType from completing inside its own UI while
...
final class SelfCaptureGateTests: XCTestCase {
    private let selfBundle = "com.yetype.app"
    private let previewIdentifier = "com.yetype.settings.context.live-preview"
    ...
    func test_self_otherField_isBlocked() {
        XCTAssertFalse(SelfCaptureGate.allowsCapture(
            focusedBundleIdentifier: selfBundle,
            ignoredBundleIdentifier: selfBundle,
            focusedElementIdentifier: "com.yetype.settings.search",
            sanctionedElementIdentifier: previewIdentifier
        ))
    }
