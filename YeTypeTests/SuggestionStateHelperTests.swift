...
final class ContextBufferTests: XCTestCase {
    ...
    func test_materialize_assignsFirstGenerationAndStoresCurrentContext() {
        runOnMainActor {
            ...
            let snapshot = YeTypeTestFixtures.focusedInputSnapshot(precedingText: "Hello")
    ...
    func test_materialize_keepsGenerationForIdenticalProcessAndContent() {
        runOnMainActor {
            ...
            let first = buffer.materialize(
                from: YeTypeTestFixtures.focusedInputSnapshot(
    ...
    func test_materialize_incrementsGenerationWhenContentChanges() {
        runOnMainActor {
            let buffer = makeBuffer()
            let first = buffer.materialize(from: YeTypeTestFixtures.focusedInputSnapshot(precedingText: "Hello"))
            let second = buffer.materialize(from: YeTypeTestFixtures.focusedInputSnapshot(precedingText: "Hello!"))
    ...
    func test_materialize_incrementsGenerationWhenProcessChanges() {
        runOnMainActor {
            let buffer = makeBuffer()
            let first = buffer.materialize(
                from: YeTypeTestFixtures.focusedInputSnapshot(processIdentifier: 123)
            )
            let second = buffer.materialize(
                from: YeTypeTestFixtures.focusedInputSnapshot(processIdentifier: 456)
            )
...
final class SuggestionInteractionStateTests: XCTestCase {
    ...
    func test_startSessionStoresActiveSessionAndClearsPendingInsertionSentinel() {
        runOnMainActor {
            let state = makeState()
            let context = YeTypeTestFixtures.focusedInputContext(precedingText: "Hello")
        ...
        runOnMainActor {
            ...
            let preparation = state.prepareAcceptance(
                ...
                overlayState: .visible(
                    text: " world again",
                    geometry: YeTypeTestFixtures.overlayGeometry(caretRect: context.caretRect),
                    mode: .inline
                ),
                granularity: .word
            )
    ...
    func test_prepareAcceptance_rejectsVisibleOverlayTextMismatch() {
        runOnMainActor {
            ...
            let preparation = state.prepareAcceptance(
                from: YeTypeTestFixtures.focusedInputSnapshot(precedingText: "Hello"),
                overlayState: .visible(
                    text: " different",
                    geometry: YeTypeTestFixtures.overlayGeometry(caretRect: context.caretRect),
                    mode: .inline
    ...
    func test_prepareAcceptance_allowsHiddenOverlayWhenProcessStillMatches() {
        runOnMainActor {
            let state = makeState()
            let context = YeTypeTestFixtures.focusedInputContext(
                processIdentifier: 123,
                precedingText: "Hello"
            )
            _ = state.startSession(fullText: " world again", liveContext: context, latency: 0.1)
    ...
    func test_commitAcceptedChunkAdvancesSessionAndSetsPendingInsertionSentinel() {
        runOnMainActor {
            let state = makeState()
            let context = YeTypeTestFixtures.focusedInputContext(precedingText: "Hello", generation: 7)
    ...
    func test_commitAcceptedChunkClearsSessionWhenTailIsExhausted() {
        runOnMainActor {
            let state = makeState()
            let context = YeTypeTestFixtures.focusedInputContext(precedingText: "Hello", generation: 4)
    ...
    func test_reconcileActiveSessionStoresAdvancedSession() {
        runOnMainActor {
            ...
            let reconciliation = state.reconcileActiveSession(
                with: YeTypeTestFixtures.focusedInputSnapshot(precedingText: "Hello world")
            )
...
final class SuggestionOverlayPresenterTests: XCTestCase {
    func test_presentEmptyTextHidesOverlay() {
        runOnMainActor {
            ...
            let message = presenter.present(
                text: "   ",
                geometry: YeTypeTestFixtures.overlayGeometry(caretRect: .zero),
    ...
    func test_presentNewTextShowsOverlayAndReturnsDisplayedMessage() {
        runOnMainActor {
            ...
            let message = presenter.present(
                text: " world",
                geometry: YeTypeTestFixtures.overlayGeometry(caretRect: caretRect),
                previousState: overlayController.state
            )
    ...
    func test_presentIdenticalVisibleStateDoesNotCallOverlayAgain() {
        runOnMainActor {
            let caretRect = CGRect(x: 10, y: 20, width: 2, height: 18)
            let geometry = YeTypeTestFixtures.overlayGeometry(caretRect: caretRect)
            let overlayController = FakeOverlayController(
    ...
    func test_presentSameTextAtNewCaretReturnsMovedMessage() {
        runOnMainActor {
            ...
            let message = presenter.present(
                text: " world",
                geometry: YeTypeTestFixtures.overlayGeometry(caretRect: nextRect),
                previousState: .visible(
                    text: " world",
                    geometry: YeTypeTestFixtures.overlayGeometry(caretRect: previousRect),
                    mode: .inline
                )
            )
    ...
    func test_presentSameTextAndCaretWithNewQualityReturnsStylingMessage() {
        runOnMainActor {
            ...
            let message = presenter.present(
                text: " world",
                geometry: YeTypeTestFixtures.overlayGeometry(
                    caretRect: caretRect,
