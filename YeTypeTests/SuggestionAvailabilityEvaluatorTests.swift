...
final class SuggestionAvailabilityEvaluatorTests: XCTestCase {
    ...
    private func makeIntegratedTerminalSnapshot() -> FocusSnapshot {
        let context = YeTypeTestFixtures.focusedInputSnapshot(
    ...
    func test_disabledReason_integratedTerminal_suppressedByDefault() {
        ...
        XCTAssertEqual(reason, "YeType is not available in the integrated terminal.")
    }
    ...
    func test_disabledReason_whenGloballyDisabled_returnsFixedCopy() {
        ...
        XCTAssertEqual(reason, "YeType is turned off.")
    }
    ...
    func test_disabledReason_whenFocusedDomainIsDisabled_returnsSiteReason() {
        ...
        XCTAssertEqual(reason, "YeType is disabled on bank.com.")
    }
    ...
    /// YeType is off, the Input Monitoring message is a distraction.
    ...
    func test_disabledReason_whenAppDisabled_returnsAppSpecificCopy() {
        ...
        XCTAssertEqual(reason, "YeType is disabled in Safari.")
    }

    // MARK: - disabledReason: capability passthrough

    /// The .blocked and .unsupported cases both surface their own reason
    /// string so the menu can explain which field YeType is refusing to
    ...
    func test_disabledReason_blockedCapability_returnsCapabilityReason() {
        let blockReason = "Secure field — YeType intentionally won't run here."
        let reason = SuggestionAvailabilityEvaluator.disabledReason(
            globallyEnabled: true,
            inputMonitoringGranted: true,
...
/// not pixels; it is the invariant that YeType's own transient focus does not become the app rule
/// target after the user opens the menu bar.
final class FocusSnapshotExternalApplicationIdentityTests: XCTestCase {
    func test_externalApplicationIdentity_returnsNonYeTypeApplication() {
        ...
        XCTAssertEqual(
            snapshot.externalApplicationIdentity(ignoredBundleIdentifier: "com.kludne.yetype"),
    ...
    func test_externalApplicationIdentity_ignoresYeTypeApplication() {
        let snapshot = FocusSnapshot(
            applicationName: "YeType",
            bundleIdentifier: "com.kludne.yetype",
