...
/// still consistent with the active ghost-text tail or whether YeType must invalidate the session.
final class SuggestionSessionReconcilerTests: XCTestCase {
    func test_advanceIfTypedCharactersMatch_advancesMatchingDirectText() {
        let session = YeTypeTestFixtures.activeSession(fullText: " world again")
    ...
    func test_overlayHideReason_mapsSemanticInputEventsToUserVisibleReasons() {
        XCTAssertEqual(
            SuggestionSessionReconciler.overlayHideReason(
                for: YeTypeTestFixtures.inputEvent(kind: .textMutation)
            ),
            "Overlay hidden because typing invalidated the current suggestion."
        )
        XCTAssertEqual(
            SuggestionSessionReconciler.overlayHideReason(
                for: YeTypeTestFixtures.inputEvent(kind: .navigation)
            ),
            "Overlay hidden because caret navigation invalidated the current suggestion."
        )
        XCTAssertEqual(
            SuggestionSessionReconciler.overlayHideReason(
                for: YeTypeTestFixtures.inputEvent(kind: .dismissal)
            ),
    ...
    func test_reconcile_validWhenLiveContextStillMatchesBaseContext() {
        let session = YeTypeTestFixtures.activeSession(
            fullText: " world again",
            basePrecedingText: "Hello",
            baseTrailingText: " tail"
        )
        let liveContext = YeTypeTestFixtures.focusedInputContext(
            precedingText: "Hello",
            trailingText: " tail"
        )
    ...
    func test_reconcile_invalidWhenProcessChanges() {
        let session = YeTypeTestFixtures.activeSession(processIdentifier: 123)
        let liveContext = YeTypeTestFixtures.focusedInputContext(processIdentifier: 456)
    ...
    func test_reconcile_invalidWhenPrefixAnchorChangesOutsideInsertionSyncWindow() {
        ...
        let liveContext = YeTypeTestFixtures.focusedInputContext(precedingText: "Goodbye")
    ...
    func test_reconcile_invalidWhenConsumedSuffixDivergesFromSuggestion() {
        ...
        let liveContext = YeTypeTestFixtures.focusedInputContext(precedingText: "Hello there")
    ...
    func test_reconcile_advancesSessionWhenLiveTextConsumedSuggestionPrefix() {
        ...
        let liveContext = YeTypeTestFixtures.focusedInputContext(precedingText: "Hello world")
