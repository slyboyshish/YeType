...
final class PromptContextSanitizerTests: XCTestCase {
    ...
    func test_sanitizeOCR_dropsRandomMixedCaseAndAlphanumericGarbage() {
        ...
        Visible task update Screen Recording copy for YeType
        """
        ...
        XCTAssertTrue(result.contains("Visible task update Screen Recording copy for YeType"))
    }

    func test_sanitizeOCR_preservesUsefulTechnicalAndUserContext() {
        let input = """
        YeType PR API context needs GeneralPaneView.swift normalizedBundleIdentifier jane@example.com
        """

        let result = PromptContextSanitizer.sanitizeOCR(input)

        XCTAssertTrue(result.contains("YeType"))
