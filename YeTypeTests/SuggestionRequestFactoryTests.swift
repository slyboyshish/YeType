...
final class SuggestionRequestFactoryTests: XCTestCase {
    ...
    func test_buildRequest_truncatesPrefixByCharacterAndWordBudgets() {
        ...
        let result = SuggestionRequestFactory.buildRequest(
            context: context,
            settings: YeTypeTestFixtures.settingsSnapshot(),
    ...
    func test_buildRequest_appliesFoundationModelPrefixBudgetWhenAppleEngineSelected() {
        ...
        let context = YeTypeTestFixtures.focusedInputContext(precedingText: precedingText)
        let configuration = SuggestionConfiguration(
            maxPredictionTokens: 8,
            debounceMilliseconds: 0,
            temperature: 0.1,
        ...
        let llamaResult = SuggestionRequestFactory.buildRequest(
            context: context,
            settings: YeTypeTestFixtures.settingsSnapshot(selectedEngine: .llamaOpenSource),
            configuration: configuration
        )
        let foundationModelResult = SuggestionRequestFactory.buildRequest(
            context: context,
            settings: YeTypeTestFixtures.settingsSnapshot(selectedEngine: .appleIntelligence),
            configuration: configuration
        )
    ...
    func test_buildRequest_usesWordCountPresetForInstructionAndTokenBudget() {
        let context = YeTypeTestFixtures.focusedInputContext(precedingText: "Hello world")
        let configuration = SuggestionConfiguration(
        ...
        let result = SuggestionRequestFactory.buildRequest(
            context: context,
            settings: YeTypeTestFixtures.settingsSnapshot(selectedWordCountPreset: .twelveToTwenty),
            configuration: configuration
        )
    ...
    func test_buildRequest_carriesClipboardContextWhenEnabled() {
        ...
        let result = SuggestionRequestFactory.buildRequest(
            context: context,
            settings: YeTypeTestFixtures.settingsSnapshot(isClipboardContextEnabled: true),
            configuration: .standard,
    ...
    func test_buildRequest_omitsClipboardContextWhenDisabled() {
        ...
        let result = SuggestionRequestFactory.buildRequest(
            context: context,
            settings: YeTypeTestFixtures.settingsSnapshot(isClipboardContextEnabled: false),
            configuration: .standard,
            clipboardContext: "Copied project notes."
        )
