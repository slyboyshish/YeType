...
final class SuggestionModelValueTests: XCTestCase {
    ...
    func test_ghostSuggestionLayoutWrapsOverflowToInputLeftEdge() {
        let geometry = YeTypeTestFixtures.overlayGeometry(
...
final class RuntimeAndInputModelValueTests: XCTestCase {
    ...
    func test_capturedInputEventComputedPropertiesReflectSchedulingPolicy() {
        XCTAssertTrue(YeTypeTestFixtures.inputEvent(kind: .textMutation).shouldSchedulePrediction)
        XCTAssertTrue(YeTypeTestFixtures.inputEvent(kind: .shortcutMutation).shouldSchedulePrediction)
        XCTAssertFalse(YeTypeTestFixtures.inputEvent(kind: .navigation).shouldSchedulePrediction)

        XCTAssertTrue(YeTypeTestFixtures.inputEvent(kind: .dismissal).shouldClearSuggestion)
        XCTAssertFalse(YeTypeTestFixtures.inputEvent(kind: .acceptance).shouldClearSuggestion)
        XCTAssertFalse(YeTypeTestFixtures.inputEvent(kind: .fullAcceptance).shouldClearSuggestion)
        XCTAssertFalse(YeTypeTestFixtures.inputEvent(kind: .fullAcceptance).shouldSchedulePrediction)
        XCTAssertFalse(YeTypeTestFixtures.inputEvent(kind: .other).shouldClearSuggestion)
    }
}
