...
final class SuggestionSettingsStoreTests: XCTestCase {
    ...
    func test_load_migratesRetiredShortPresetToFourToSeven() async {
        ...
        defaults.set("3-7", forKey: "yetypeSelectedWordCountPreset")
    ...
    func test_load_prefersIndicatorModeOverLegacyBool() async {
        ...
        defaults.set(ActivationIndicatorMode.hidden.rawValue, forKey: "yetypeSelectedIndicatorMode")
        defaults.set(true, forKey: "yetypeShowCaretIndicator")
    ...
    func test_load_fallsBackToLegacyBoolWhenModeAbsent() async {
        let defaults = makeIsolatedDefaults()
        defaults.set(false, forKey: "yetypeShowCaretIndicator")
    ...
    func test_load_writesBothIndicatorKeysBack() async {
        ...
        XCTAssertNotNil(defaults.string(forKey: "yetypeSelectedIndicatorMode"))
        XCTAssertNotNil(defaults.object(forKey: "yetypeShowCaretIndicator"))
    }
    ...
    func test_load_capsPersistedDebounceAtShippedDefault() async {
        let defaults = makeIsolatedDefaults()
        defaults.set(999, forKey: "yetypeDebounceMilliseconds")
    ...
    func test_load_capsPersistedFocusPollAtShippedDefault() async {
        let defaults = makeIsolatedDefaults()
        defaults.set(999, forKey: "yetypeFocusPollIntervalMilliseconds")
    ...
    func test_load_customRulesPresentHonoredVerbatim() async {
        let defaults = makeIsolatedDefaults()
        defaults.set(["Write in the user's voice"], forKey: "yetypeCustomRules")
    ...
    func test_load_scrubsLegacyCustomIndicatorImageData() async {
        let defaults = makeIsolatedDefaults()
        defaults.set(Data([0x1, 0x2, 0x3]), forKey: "yetypeCustomIndicatorImageData")

        _ = SuggestionSettingsStore(userDefaults: defaults).load(configuration: .standard)

        XCTAssertNil(defaults.data(forKey: "yetypeCustomIndicatorImageData"))
    }
    ...
    func test_load_clampsOutOfRangeGhostTextOpacity() async {
        ...
        defaults.set(0.01, forKey: "yetypeGhostTextOpacity")
    ...
    func test_load_clampsOutOfRangeGhostTextSizeMultiplier() async {
        ...
        defaults.set(5.0, forKey: "yetypeGhostTextSizeMultiplier")
    ...
    func test_load_spellingDictionariesFiltersUnknownCodesAndStabilizesOrder() async {
        let defaults = makeIsolatedDefaults()
        defaults.set([" ru ", "unknown", "de", "ru"], forKey: "yetypeEnabledSpellingDictionaryCodes")

        let data = SuggestionSettingsStore(userDefaults: defaults).load(configuration: .standard)

        XCTAssertEqual(data.enabledSpellingDictionaryCodes, ["de", "ru"])
        XCTAssertEqual(
            defaults.stringArray(forKey: "yetypeEnabledSpellingDictionaryCodes"),
            ["de", "ru"]
        )
    }
    ...
    func test_load_spellingDictionariesPreservesExplicitEmptySelection() async {
        let defaults = makeIsolatedDefaults()
        defaults.set([String](), forKey: "yetypeEnabledSpellingDictionaryCodes")
    ...
    func test_load_legacyModelOnlyProfilePreservedWithOpenSourceEngine() async {
        ...
        defaults.set("small-model.gguf", forKey: "yetypeBatteryModelFilename")
        defaults.set("big-model.gguf", forKey: "yetypePluggedInModelFilename")
    ...
    private func makeIsolatedDefaults() -> UserDefaults {
        let suiteName = "yetype.test.settingsStore.\(UUID().uuidString)"
