...
DERIVED_DATA="/tmp/YeTypeDerivedData"
APP_PATH="$DERIVED_DATA/Build/Products/Debug/YeType.app"
OUTPUT_PATH="/tmp/YeType-test.dmg"
BACKGROUND="$REPO_ROOT/assets/release/dmg_background.png"
BACKGROUND_2X="$REPO_ROOT/assets/release/dmg_background@2x.png"
VENV_DIR="/tmp/YeType-dmg-venv"
...
if [ ! -d "$APP_PATH" ]; then
    echo "YeType.app not found, building..."
    xcodebuild \
        -project "$REPO_ROOT/YeType.xcodeproj" \
        -scheme YeType \
        -configuration Debug \
...
# Eject any stale YeType volumes before building so the DMG mounts cleanly.
# The DS_Store background path is absolute: if it mounts as /Volumes/YeType 2/
