#!/usr/bin/env python3
"""Build a styled release DMG for YeType.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Build a styled YeType release DMG with dmgbuild."
    )
    parser.add_argument(
        "--app-path",
        required=True,
        help="Path to the signed YeType.app bundle that should be packaged.",
    )
    parser.add_argument(
        "--output-path",
        required=True,
        help="Path where the final YeType.dmg should be written.",
    )
    parser.add_argument(
        "--background-path",
        required=True,
    ...
    parser.add_argument(
        "--volume-name",
        required=True,
        help="Mounted volume name shown by Finder, for example YeType.",
    )
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="YeType-dmgbuild-") as temporary_root:
