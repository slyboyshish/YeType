#!/usr/bin/env python3
"""Generate a Sparkle appcast entry for a notarized YeType DMG.

This script intentionally does not depend on Sparkle's `generate_appcast` helper because YeType's
release flow is small and predictable:
1. a notarized `YeType.dmg` is uploaded to GitHub Releases
...
DEFAULT_OWNER = "slyboyshish"
DEFAULT_REPOSITORY = "YeType"
...
DEFAULT_RELEASE_NOTES_BASE_URL = "https://yetype.app/release-notes"
SIGNATURE_PATTERN = re.compile(r'sparkle:edSignature="([^"]+)"\s+length="([^"]+)"')


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate YeType's Sparkle appcast XML.")
    ...
    parser.add_argument("--archive", required=True, help="Path to the notarized YeType.dmg")
    parser.add_argument(
        "--output",
