#!/usr/bin/env ruby
# Adds a `YeTypeTests` unit-test target to YeType.xcodeproj and wires the
# existing Swift files in `YeTypeTests/` into it. Idempotent — safe to re-run.
#
...
PROJECT_PATH      = 'YeType.xcodeproj'
HOST_TARGET_NAME  = 'YeType'
TEST_TARGET_NAME  = 'YeTypeTests'
DEPLOYMENT_TARGET = '26.0'
TEST_BUNDLE_ID    = 'com.kludne.yetype.YeTypeTests'
TESTS_DIR         = 'YeTypeTests'
...
# test time; this is what makes `@testable import YeType` actually resolve.
...
test_target.build_configurations.each do |config|
  config.build_settings.merge!(
    ...
    'TEST_HOST'                    => '$(BUILT_PRODUCTS_DIR)/YeType.app/Contents/MacOS/YeType',
...
# Without this, `xcodebuild test -scheme YeType` has nothing to run.
