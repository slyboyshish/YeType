...
- Feed: https://updates.yetype.app/appcast.xml
...
### Backup private key
```sh
sparkle-generate-keys -x ~/secure/YeType-key.txt
```

### Import on another machine
```sh
sparkle-generate-keys -f ~/secure/YeType-key.txt
```
...
### 2. Notarize
```sh
ditto -c -k --keepParent ./YeType.app YeType.zip
xcrun notarytool submit YeType.zip --keychain-profile "AC_PASSWORD" --wait
xcrun stapler staple ./YeType.app
```
...
python3 scripts/build_release_dmg.py \
  --app-path /path/to/YeType.app \
  --output-path /path/to/YeType.dmg \
...
What this does:
- packages `YeType.app` with an `Applications` shortcut
...
```sh
sparkle-sign-update /path/to/YeType.dmg
```
...
python3 scripts/generate_appcast.py \
  ...
  --build-number 100 \
  --archive /path/to/YeType.dmg \
  --output build/appcast.xml \
  --ed-key-file ~/secure/YeType-key.txt
```
...
4. Packages a styled `YeType.dmg` with `scripts/build_release_dmg.py`.
...
9. Creates a GitHub Release with `YeType.dmg`.
10. Publishes `appcast.xml` to GitHub Pages last.

Pages output:
- `/appcast.xml`

The `/appcast.xml` path matches the current feed URL (`https://updates.yetype.app/appcast.xml`).
...
```sh
hdiutil attach /path/to/YeType.dmg
```

Verify the mounted image opens in icon view, shows the committed background
art, places `YeType.app` above the arrows, and places the `Applications` shortcut
