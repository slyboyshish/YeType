# YeType Codex Instructions

## Project Identity

YeType is a macOS menu bar app for on-device inline autocomplete. The core loop is:
...
- `YeType/App/`: app entrypoint, composition root, lifecycle wiring, and coordinators.
- `YeType/UI/`: SwiftUI/AppKit presentation: settings, onboarding, menu views, overlays, and
  visual affordances.
- `YeType/Services/`: side-effectful boundaries: Accessibility, input monitoring, text insertion,
  screenshots/OCR, visual context, llama runtime, permissions, downloads, updates, and launch
  services.
- `YeType/Models/`: shared value types, settings snapshots, states, domain models, and protocol
  contracts.
- `YeType/Support/`: pure helper logic, prompt rendering, availability rules, normalization,
  reconciliation, geometry helpers, and low-level bridging utilities.
- `YeTypeTests/`: unit and microbench tests. Prefer testing pure `Support/` and `Models/` logic
  when possible.
- `YeTypeInference`: the llama.cpp wrapper, consumed as a SwiftPM package
  (`github.com/slyboyshish/YeType`, pinned to `main`) rather than vendored in-tree.
...
`YeTypeAppEnvironment` builds the long-lived dependency graph once. `AppDelegate` starts, stops,
...
- `FocusSnapshotResolver`: reduces raw AX elements into YeType-supported focus snapshots.
...
- `LlamaRuntimeCore`: serialized actor around mutable llama.cpp pointers, prompt tokenization,
  ...
  (`runConstrainedDecode`, gated behind the default-off `yetypeConstrainedDecoderEnabled`), and
  shutdown.
...
- Settings panes (under `YeType/UI/Settings/Panes/`) and onboarding views should remain
...
YeType has a structured logging system built for AI-assisted debugging. During development the app
is launched with `-yetype-debug`, which enables on-disk JSONL sinks in addition to the always-on
Console.app stream.

**Log file locations** (only populated when `-yetype-debug` is set):

- `~/Library/Logs/YeType/yetype.jsonl` — main event stream. One JSON object per line, with all
...
**Console.app fallback** (when `-yetype-debug` wasn't set):
...
`~/Library/Developer/Xcode/DerivedData/YeType-*`, where every build leaves a fresh multi-GB module
